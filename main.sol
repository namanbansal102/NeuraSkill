    ...
    hacakthon[] public hackathons_arr;// we have created a hacakthon array
        ...
        function registerHackathon(string memory _title,string memory _img_url,string memory _desc,uint  _prizePool,string memory _st_date,string memory _end_date,string memory _mode,uint _nAwards)  public {
            hackathons_arr.push(hacakthon(hack_id,msg.sender,_title,_img_url,_desc,_prizePool,_nAwards,_st_date,_end_date,_mode,0));
            hack_id++;
        }
        ...
        function getAllWinners(uint _hack_id) private view returns(uint[] memory) {
            uint nAwards=hackathons_arr[_hack_id].nAwards;
            uint[] memory winners=new uint[](nAwards) ;
            ...
            for (uint256 i = 0; i < my_pairs_array.length; i++) {
                ...
                winners[i]=(my_pairs_array[i].project_id);
                nAwards--;
            }
        ...
        function end_hackathon(uint _hack_id)external payable{
            ...
            // given the prize pool to winner 
            // and some amount to investors in project Parts
            // we have to predict the first second and third prize of my  winning competition
            // there is no condition for handling the same upvotes
            // then upvote to all my all my contributors
            // uint[] memory winners=getAllWinners(_hack_id);
            // and all 
        }
    }