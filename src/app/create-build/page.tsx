  ...
  return (
    ...
    <button className="bg-white" onClick={()=>{
      ...
      Get Prize pool
              ...
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
                <label className="block mb-2">Project Github Id</label>
                <input
                  type="text"
                  value={formData.externalLink}
                  onChange={(e) => handleInputChange("externalLink", e.target.value)}
                  className="w-full bg-white/5 border border-gray-800 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
                  placeholder="https://github.com/namanbansal102/neoxlifechain"
                />
              </motion.div>
              ...
              {prizes.map((prize, index) => (
                <motion.div key={prize.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
                  <label className="block mb-2">Team Member {index + 1} (Metamask)</label>
                  <input
                    type="text"
                    value={prize.amount}
